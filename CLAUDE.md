...
**Always read WNS *per clock*, not design-wide — the two differ by 65x here.** Iter66e's design-wide WNS is +0.003 ns, and it is easy to read that as "the kernel barely closed." It did not: `clk_kernel_00_unbuffered_net` has **+0.195 ns** over 1,580,815 endpoints, and the +0.003 belongs to the fixed 250 MHz `dma_ip_axi_aclk_1`, which raising the kernel clock does not directly load. Vivado's own `report_qor_suggestions` on the routed checkpoint says it has no suggestions because the design "is assessed to easily meet timing." **Frequency is therefore an open lever from 100 MHz toward roughly 200 MHz, not an unlimited clean multiplier.** One 256-bit/450-MHz HBM pseudo-channel peaks at 14.4 GB/s; a 512-bit kernel port demands 12.8 GB/s at 200 MHz and 16.0 GB/s at 250 MHz. Port occupancy is approximately frequency-invariant only below that per-channel wall. See `c_impl/doc/frequency_250mhz_roadmap.md` for the measured-byte floor and required 200/250 MHz microbenchmark.
