...
3. Only after an iteration demonstrates a real improvement: update `doc/architecture.md`, the relevant block doc, **and `cycle_optimization_roadmap.md`** (AGENTS.md requires the roadmap in the same positive commit series — record explicitly when an iteration closes *no* stage, as Iter73–75b did, rather than leaving it silent), then commit the source, build/config/Tcl, and all accumulated log entries in focused commits. Never commit build products, logs, weight blobs, or `.gdnstate`.
...
*Last updated 2026-09-12. Every number below is on-card unless labelled otherwise.*

| | |
|---|---|
| Shipping image | **150 MHz production image of the Iter76 `make run_hw` flow** (build 4022 → XCLBIN `b2a0a478…`; build 3987 and closure job 3955 are checksum-identical implementations; the 3956 image `82aa21e8…` carries the drift/WikiText/power qualification). Iter67c datapath + Iter73a/73b2/75b source changes, Vitis 2024.2, true 6.667 ns constraint, no exceptions, no auto-scaling |
| TPOT | **16.255 ms/token production** (token-id to token-id) and **16.131 ms kernel**, median of 63 (job 4023) at a verified 150 MHz = 2.4197M kernel cycles; host/XRT/PCIe overhead 0.124 ms. Kernel-only vs Iter67c: 24.099 → 16.131 ms, **−33.1%**, on 0.4% *more* cycles — the gain is clock. Iter67c's 24.208 / 24.099 (job 3101) remains the 100 MHz reference |
| Routed timing | 1,592,229/1,592,229 nets routed, 0 routing errors. **Kernel clock 0.000 ns setup / +0.002 ns hold at 6.667 ns**, DMA +0.003 / +0.009, HBM +0.052 / +0.010; 0 failing of 2,039,265 setup and 2,036,610 hold endpoints. Closure ladder inside the link −0.013 → −0.001 → 0.000, identical on every build that reached it (3953/3955, 3990, 3987, 4022). Read WNS **per clock** |
| Correctness | exact 64-token trajectory; CUDA vector gate over 2,016,000 logits — NRMSE 0.00466, min cosine 0.99995, top-5 exact, zero argmax mismatches — bit-identical figures on all three 150 MHz images and on Iter67c |
| Long-run drift | bounded to 512 tokens; hardware tracks the GPU for 447 tokens (job 3960 on the 3956 image, the same fork step as every prior image) |
| Per-token weight bytes | 2.799 GB → 1,366,528 beat-cycles/port = **56.5%** port occupancy of the 2,419,650-cycle token; a busy port draws 9.6 GB/s = 66.7% of its 14.4 GB/s pseudo-channel peak |
| Resources (csynth) | BRAM18 1,995 (49%) · DSP 3,453 (38%) · FF 888K (34%) · LUT 821K (62%) · URAM 112 (11%) — build 4022's csynth (Iter67c: 1,995 / 3,325 / 1.00M / 895K / 80) |
| Routed per-SLR CLB | **97.12%** / 75.77% / 77.78% · SLR1↔SLR0 SLL 85.72%, SLR2↔SLR1 57.36% (Iter67c was 99.29 / 87.51 / 81.97, SLL 84.76; Iter66e 96.53 / 94.68 / 76.09, SLL 89.57) |
| Task quality | WikiText-2 on card (job 3960, 3956 image): word PPL **16.774839771** vs GPU **16.776123769** on verified-identical windows = **−0.0077%**, gate 5% — reproducing the Iter66o 100 MHz result to 9 significant figures. Power (jobs 3958/3959): 48.0 W active, 26.0 W idle, **0.779 J/token** gross, 0.357 J idle-subtracted; 1.94× / 3.88× / 2.30× vs the paired A100 FP32 run |
| Committed | Yes — the flow (`56cd28118`) and its record (`26abeacf4`) are committed; `make run_hw` defaults to HLS 150 / link 150 / `hw_f150.cfg`. Iter67c (`caf512543`) is the 100 MHz predecessor. Auto-scaled, unpromoted images remain on record only: Iter69 (121 MHz), Iter72 r2 (137.7), Iter75b (142.5) |
| In flight | Nothing. Iter76 closed 2026-09-12 as **retained**. Open questions recorded, not being worked: whether the placer's physical-synthesis divergence under foreign node load (one sample, build 3968) is causal — the recommended launch is `BUILD_EXCLUSIVE=user`; and whether the finishing hook could regenerate vpl's post-route timing report so the XCLBIN metadata comes out at 150 natively (unverified; `reconcile_exact_clock.py` is the fix of record). Next levers are the cycle roadmap's (sub-byte weights, the recurrent block), not clock |
...
| Document | Status |
|---|---|
| `c_impl/doc/architecture.md` | **Authoritative** description of the production kernel (currently the **150 MHz image of the Iter76 flow**; Iter67c is its 100 MHz predecessor) — data flow, HBM map, per-layer schedule, physical design, measured result. Start here. |
...
| `c_impl/doc/cycle_optimization_roadmap.md` | **Proposed** future stages. Targets, not implemented hardware. Its cycle reference is deliberately still **Iter67c's 2.4099M**, not the 150 MHz production image, because that image's cycles are 0.4% *worse* (2.4197M) — every gain above 100 MHz is clock, not schedule, so frequency and cycle levers are not additive. |
| `c_impl/doc/frequency_250mhz_roadmap.md` | **Historical.** Record of the Iter68 frequency-locality redesign (closed 2026-09-05 as stopped/inconclusive) and the HBM-aware frequency floor model. 250 MHz is no longer a target. |
...
Two weight sizes, both correct, easy to confuse: the `.gdnw` blob on disk is a **5.87 GB FP32-container** file whose values are BF16-exact, while the device holds **32 packed-BF16 shards of 1,366,528 beats (87.5 MB each)** carrying 1,399,324,672 dense parameters with no padding = **2.799 GB of weight bytes per token**. That is the five hidden-size projections per layer (`q/k/v/g/o_proj`) plus `gate/up/down_proj`, ×24, plus `lm_head`; the host keeps the embedding table. Verified from the checkpoint shapes and from the reader bound `rows_per_ch * k_packs` summed over the 97 GEMV calls. **Docs before 2026-09-08 said 1,298,661,376 / 2.597 GB / 1,268,224 beats — that dropped one 2048×2048 projection per layer.**
...
```
The GPU prefills a prompt and exports the fixed-size recurrent+conv state with `scripts/export_gdn_state.py` → `.gdnstate` (~52 MB); the FPGA decodes from it through the `gdn_gemv` engine. The decode fixture (`fixtures_decode/decode.gdnreq`) and GPU golden (`results_decode_golden/`) are committed, but the `.gdnstate` files are **gitignored/regenerable** (like the `.gdnw` weight blob) — so the gate and the standing hook require both to be generated locally first (`export_gdn_state.py` + `export_gdn_c.py weights`). A PostToolUse hook in `.claude/settings.json` auto-runs the fast check (and `.claude/settings.local.json` wires a second hook script, `.claude/hooks/post-edit-parity.sh`, on the same events, so one edit may run it twice) on every edit to `c_impl/gdn_model.{cpp,h}`, `host.cpp`, `gdn_eval.cpp`, `lit_gpt/gated_delta_net.py` — keep it passing.
...
**`make run_hw` is the production recipe, and on the login node it submits the two chained Slurm jobs itself** (through `run_hw_sbatch.sh` — see *Cluster environment* below; a card and 48 cores cannot be held by one job). The build job freezes a source snapshot, runs the native, XO-architecture and synthesized state-address gates, builds HLS at 150 MHz, links at a true 150 MHz with the in-link closure hook and fail-closed exact-clock gate, reconciles the clock metadata, and copies the image back; the `afterok` card job runs the exact 8-token and 64-token gates with the frozen `reproduction.Makefile` (`GDN_ONCARD=1`). Demonstrated twice from source (builds 3987 and 4022, Iter76). Do not add iteration-specific Make targets or launcher scripts; preserve historical commands in `optimization_log.md` instead.

**Kernel frequency:** `HLS_FREQ`, `LINK_FREQ` and `FREQ` all default to **150 MHz** (since Iter76, 2026-09-12; through Iter67c the link default was 100). The U55C platform defaults to 300 MHz, which this kernel cannot meet, so never omit the link override. A requested frequency is not an achieved frequency: verify `DATA_CLK` in the XCLBIN (`xclbinutil --input <xclbin> --info`) and the implemented period in the routed `timing_summary.rpt`, and report the per-clock WNS. **Vitis 2024.2's `v++` link implements the kernel clock at 10.000 ns regardless of `--kernel_frequency`** — vpl's clkwiz query returns a 1:1 ratio into `_user_impl_clk.xdc`, so `LINK_FREQ=150` alone produced a 100 MHz implementation (Iter68G, read from the checkpoints' constraint files; 2022.2 is not affected, and 100 MHz requests were unaffected because 1:1 *is* 100 MHz). The production `hw_f150.cfg` inherits the fix (`apply_iter69_kernel_clock_f150.tcl` overrides that XDC at OPT_DESIGN.PRE), and `reconcile_exact_clock.py` repairs the second symptom: vpl's AUTO-FREQ-SCALING writes the frequency it derives from its *own pre-hook* timing report into the XCLBIN (149 on builds 3987 and 4022) even after the finishing hook has closed the design; the tool patches DATA_CLK to the request only when the hook's gate file proves every clock non-negative. The fixed 250 MHz `dma_ip_axi_aclk_1` is a separate timing gate even when the scalable kernel clock closes.

**Always read WNS *per clock*, not design-wide — the two differ by 65x here.** Iter66e's design-wide WNS is +0.003 ns, and it is easy to read that as "the kernel barely closed." It did not: `clk_kernel_00_unbuffered_net` has **+0.195 ns** over 1,580,815 endpoints, and the +0.003 belongs to the fixed 250 MHz `dma_ip_axi_aclk_1`, which raising the kernel clock does not directly load. Vivado's own `report_qor_suggestions` on the routed checkpoint says it has no suggestions because the design "is assessed to easily meet timing." **Frequency was then measured (Iter69–72, Sept 2026) and it is a wire wall, not open headroom.** The Iter67c netlist at a true 6.667 ns constraint routes legally but fails by **−1.594 ns** (job 3371, 25,528 failing endpoints) — wire and SLL congestion, not logic; auto-scaled to 121 MHz it runs at 20.103 ms kernel with identical gates. A 125 MHz link (Iter70a) was rejected. Floorplan-only re-pinning (Iter71/72) showed **no attributable gain over placer variance, which is ≥1.3 ns between draws of one recipe**: re-placing the unchanged pblocks gave −0.675 ns (V0); the seven-cluster SLR1 re-pin gave −0.592 ns in the production flow (**Iter72 r2: 137.7 MHz, 17.955 ms kernel, all gates identical, not promoted** — the recipe does not reproduce the image, only the XCLBIN does). Rules from that campaign: (1) judge a timing change by which *failing-endpoint class* disappears, not by WNS; (2) read `route_status.rpt` before any slack — Iter71 V2's "−0.476 ⇒ 140 MHz" was a placement estimate on a design that aborted at congestion level 7; (3) at a 150 MHz constraint `hbm_aclk` is a **third clock to read** — it failed by −0.026 ns on Iter72 r2 and was auto-scaled 450 → 444.8 MHz; (4) the timing-closed 150 MHz came from the Iter73–75b source work plus the Iter75d/e/f physical passes, and **Iter76 made it a reproducible single-command flow** (status block). Iter68's persistent-service redesign for 250 MHz is **closed dead** — no timing gain, four RTL deadlocks csim could not see, an on-card hang with every state write-back mis-addressed by 2^27 B — do not reopen it. The HBM ceiling still bounds what frequency can buy: one 256-bit/450-MHz pseudo-channel peaks at 14.4 GB/s; a 512-bit port demands 12.8 GB/s at 200 MHz and 16.0 GB/s at 250 MHz, and the 137.7 MHz image already spends 2.6% more cycles per token than the 100 MHz one.

**`HW_CFG_TEMPLATE` defaults to `hw_f150.cfg`** — the 150 MHz production recipe — so `BUILD_EXCLUSIVE=user BUILD_NODE=acclnode01 BUILD_EXCLUDE=acclnode04,acclnode05,harrier make -C c_impl run_hw` reproduces the routed image (`c_impl/doc/reproduce_f150.md` explains the three knobs). `hw_iter66e_frp_unpair_f100.cfg` is the 100 MHz Iter66e/Iter67c recipe and `hw_f150_physical_islands.cfg` the pre-Iter66 one, both retained for A/B comparison only.
...
- relocatable `@C_IMPL_DIR@` Tcl paths, the island/collector pblocks (Iter61 lineage, with Iter66b's cluster-10-in-SLR1 topology restored and cluster 9 left free), measured DMA/reset fanout repairs, and a structural post-place gate that fails closed on hierarchy drift. The 100 MHz recipe used `SSI_SpreadSLLs`, `NoTimingRelaxation` and pre/post-route `AggressiveExplore`; **`hw_f150.cfg` uses `SSI_SpreadLogic_high` placement, `AlternateCLBRouting`, pre/post-route `AggressiveExplore`, and then `finish_f150_timing.tcl` on `POST_ROUTE_PHYS_OPT_DESIGN.TCL.POST`**: if kernel setup is still negative, one `Explore` pass, then one focused `-path_groups` kernel pass (`-placement_opt -routing_opt -critical_cell_opt`), then the exact-clock gate (`check_iter75d_final_timing.tcl`), route status, DRC and bus skew — any failure aborts the link. Measured ladder on the production placement: −0.013 → −0.001 → 0.000 ns, identical across builds 3953/3955, 3990, 3987 and 4022.
- **Iter75d's pre-place control replication** (`apply_iter75d_control_fanout.tcl`, PLACE_DESIGN.PRE): eight measured control drivers — the SLR0 kernel reset, `gdn_gemv`'s `ap_start`, `ap_CS_fsm_reg[91]`, two RMSNorm and two output-norm address truncations, and a `gemv32_store` state bit — get `FORCE_MAX_FANOUT` 32/8; it sources the Iter66e un-pairing hook, which chains the Iter54/35/23 DMA repairs.
...
| Link config | `hw_f150.cfg` (default, 150 MHz), `hw_iter66e_frp_unpair_f100.cfg` (100 MHz Iter66e/67c), `hw_f150_physical_islands.cfg` (pre-Iter66) |
| Tcl chain | `apply_iter69_kernel_clock_f150.tcl` → *(OPT_DESIGN.PRE)*, which **sources** `apply_f150_physical_islands.tcl`; `apply_iter75d_control_fanout.tcl` → *(PLACE_DESIGN.PRE)*, which sources `apply_iter66e_unpair.tcl` → `apply_iter54_dma_timing.tcl` → `apply_iter35_dma_w15_fifoaddr_fanout.tcl` → `apply_iter23_dma_fanout.tcl`; `check_f150_physical_islands.tcl` → *(PLACE_DESIGN.POST)*; `finish_f150_timing.tcl` → *(POST_ROUTE_PHYS_OPT_DESIGN.TCL.POST)*, which sources `check_iter75d_final_timing.tcl` → `report_final_qor.tcl` |
| Gates | `check_native_bf16_xo.py` (fail-closed XO architecture gate), `check_state_writer_addresses.py` (synthesized state-address gate), `reconcile_exact_clock.py` (clock-metadata reconciliation, fail-closed), `packed_bf16_cosim_check.sh` + `cosim_all_bf16.tcl.in` + `packed_bf16_one_layer_test.cpp` (RTL cosim), `scripts/decode_correctness_check.sh`, `scripts/check_gdn_c_parity.py` |
...
**If a change touches an `m_axi` address or a memory write path, run the pre-link synthesized-address gate** (`c_impl/check_state_writer_addresses.py`, `STATE_ADDRESS_GATE=1`). csim, csynth and RTL cosim are all blind to this failure class — Vivado's *kernel* synthesis can transform an address HLS emitted correctly, and cosim simulates the HLS RTL. Iter68G and Iter73a both wrote recurrent state exactly 128 MiB below its stripe on card while passing every pre-link gate, including a one-layer cosim that seeded and checksummed the state stripe. The gate runs the same stimulus at HLS-RTL, post-synthesis and post-opt, so it localises the defect to a compiler stage in ~48 min instead of a 9–20 h link. Its testbench prints an unconditional `ADDRESS_PASS` summary even when every case failed — read the wrapper's verdict, never grep for the pass line.

**Launch, arm a watcher, stop — never poll (user rule, 2026-09-02).** Any time a long job is launched (csynth, cosim, `v++` link, Slurm build or on-card job), the launching turn does exactly three things: submit it, record its job ID / log / exit-marker, and arm a *detached* completion watcher (`Bash run_in_background` loop that waits for the exit marker **or** for the job to leave `squeue`, so it fires on FAILED/CANCELLED/TIMEOUT too). Then **end the turn**. Do not sleep-and-check, do not re-read the log "to see how it is going", and do not spend tokens summarising progress the watcher will report anyway. Act only when the watcher's notification arrives, and then read the real artifacts (`runme.log`, `xsim.log`, the reports), not the wrapper's summary. A bounded foreground `timeout ... tail -F` is a launch sentry, not a watcher — it cannot re-invoke anyone when the job finishes hours later.
...
- **Build and on-card run are two jobs, always.** `build` grants no FPGA and
  ...
  `make run_hw` on the login node *is* that submission; under Slurm the card
  job calls the frozen `reproduction.Makefile` with `GDN_ONCARD=1`, a target
  with no build prerequisites, so an 8-core job can never start a link.
...
- **One FPGA and one GPU per user across all jobs**, even though `light` now
  ...
  blaming the queue.
- **Memory.** Vivado peaks at **~64–66 GB** on the 150 MHz links (`sacct MaxRSS` 63.8 GB job 3453, 66.4 GB job 3449; Iter57 peaked 34.7 GB;
...
- **Not every node can link for the U55C.** Probed 2026-08-22: the platform is
  ...
  serialises against card jobs. **`harrier`'s node-local `/tmp` was 100% full
  on 2026-09-06** (833 of 879 GB; a link writes ~47 GB) and killed build 3449
  inside block synthesis behind a misleading `Invalid argument`; links since
  then pass `BUILD_EXCLUDE=acclnode04,acclnode05,harrier`. `hw_build.slurm`
  never deletes its `/tmp/$USER-$SLURM_JOB_ID` stage, and the Iter66e/Iter67c
  routed checkpoints exist only there — do not clean it blind.
...
- **The measured ladder** (each step on-card): 121.4 ms (8-port reference) → 98.66 (Iter32, activation resident) → 75.06 (Iter35, DMA fanout repair) → 59.58 (Iter36, head-local recurrence) → 51.45 (Iter37, four state ports) → 47.08 (Iter38, merged layouts/concurrent state) → 43.09 (Iter39C, head-streamed convolution) → 42.02 (Iter57, timing-friendly recurrent islands) → 42.17 (Iter61, on-chip logit export) → 26.65 (Iter66e, all-BF16) → 24.21 (Iter67c, on-chip argmax + II=1 recurrent read) → **16.26** (Iter76: the Iter73b2/75b source at a timing-closed 150 MHz, reproduced from source by the committed `make run_hw` flow; TPOT 16.255, kernel 16.131 ms). Total **7.47×** over the eight-port reference. Off the ladder because auto-scaled and unpromoted: 20.10 (Iter69, 121 MHz), 17.96 (Iter72 r2, 137.7 MHz) and 17.34 (Iter75b, 142.5 MHz). Steps through Iter61 are bit-exact; Iter66e onward are exact-trajectory plus the CUDA vector gate — see the gate note under *Decode correctness*.
  **Kernel-only, like for like: 25.625 → 24.099 ms (−6.0%) → 16.131 ms (−33.1%).** The last step's cycle count is 0.4% *worse* (2.4099M → 2.4197M), so all of it is clock. The ladder's earlier entries are wall times under older, looser timer boundaries, so treat the last three steps as the only strictly comparable set.

### How TPOT is measured — the boundary moved twice, so old numbers are not comparable

`host.cpp` reports two clocks per step and they answer different questions:

| clock | spans | Iter67c |
|---|---|---:|
| `kernel_ms` | `run.start()` → `run.wait()` | **24.099 ms** |
| `per_step_tpot_ms` | embedding lookup + 8 KiB upload + XRT launch/wait + full kernel incl. fused argmax + one 64-byte token read-back | **24.208 ms** |

**Production TPOT is the second one**, and it is the number to quote against a GPU baseline, because token selection is part of emitting a token and `bench_tpot.py` includes `.argmax()` inside its own timed window.

Two boundary changes happened during Iter67 and invalidate cross-version wall comparisons: the two 32,000-value `compare_logits_step` calls were moved out of the window, and then the clock was made to stop right after the token read-back so the optional 128 KB logit transfer sits outside it too. Consequence: enabling a correctness reference no longer inflates TPOT, and the gated run measures the deployed path directly — no reference-free run needed. The gap between the old and new boundary on the same image, **0.477 ms**, is precisely what the on-chip argmax saves by making the 128 KB read-back unnecessary.
...
- **At the production 150 MHz, the design is still not HBM-bound — always re-evaluate this at a new clock.** At 2.799 GB of weights per token, each port needs **1,366,528 beat-cycles against a measured 2,419,650** (16.131 ms × 150 MHz) — the ports are busy **56.5%** of the token, and a busy port demands 9.6 GB/s, **66.7%** of its 14.4 GB/s pseudo-channel peak (average over the token: 173.5 GB/s aggregate). The microbench sustains 98.353% of clock-rate ceiling on this exact port structure, so the other 43% is scheduling, not memory. This is also why BF16 delivered 1.582× and not 2× at 100 MHz (where the ports were busy 53.3% of a 2,562,500-cycle token). The standing correction is clock-scoped: halving bytes does not halve the current token, but at 200 MHz each port would request 88.9% of its pseudo-channel peak and at 250 MHz 111.1%. Compression regains first-order leverage once the design approaches that wall.
- **Open levers, in order of measured promise.** (1) **Sub-byte weight compression** (INT4/INT3, non-power-of-two groups with dequantisation in the datapath) is still the only lever with the right magnitude, and it is now the *only* one above 10%: it would take weight beats from 1,366,528 to ~341,632 per port (the 37% / 25.6 → ~16.1 ms estimate was derived from the old 1,268,224 figure and needs re-deriving), and it is something tensor cores cannot natively execute. (2) **The recurrent block is the new second target** — 43,427 cycles/layer × 24 = **1.042M cycles, 40.7% of the token**, and it did *not* shrink when the weights did. Its state movement alone (`recur_island_load_state` 1,027 + `recur_island_update` 1,035, per head, × 192 head-steps) is **395,904 cycles = 15.4%**. (3) The cycle roadmap's remaining stages — head-chunked output projection, then chunk-streamed GU/SwiGLU/MLP-down — remain percent-level. (4) **Frequency is closed at 150 MHz** (Iter76: 16.131 ms kernel, −33.1% vs Iter67c, reproducible from source). Beyond it, the per-port HBM demand (66.7% of pseudo-channel peak at 150, 88.9% at 200) and the placer-variance evidence make further clock a source-side campaign, not a recipe change. Levers (1) and (4) compound. Preserve the physical island, frp, and registered-boundary structure until a replacement completes on-card validation.
