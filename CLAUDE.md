...
3. Only after an iteration demonstrates a real improvement: update `doc/architecture.md`, the relevant block doc, **and `cycle_optimization_roadmap.md`** (AGENTS.md requires the roadmap in the same positive commit series), then commit the source, build/config/Tcl, and all accumulated log entries in focused commits. Never commit build products, logs, weight blobs, or `.gdnstate`.
