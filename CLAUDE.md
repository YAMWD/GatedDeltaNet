...
| TPOT | **24.208 ms/token production** (token-id to token-id) and **24.099 ms kernel**, both median of 63 (job 3101) at a verified 100 MHz = 2.4099M kernel cycles. Host/XRT/PCIe overhead is **0.109 ms**. Not comparable to Iter66e's 26.654/25.625: the timer boundary changed twice in between — see *How TPOT is measured* |
...
| Per-token weight bytes | 2.799 GB → 1,366,528 beat-cycles/port = **56.7%** port occupancy (of Iter67c's 2,409,900 cycles) |
...
| Committed | Yes — **Iter67c** is HEAD (`caf512543`) and the `run_hw` default. Two faster images exist and are **not promoted**: Iter69 (121 MHz, 20.103 ms kernel, job 3372) and **Iter72 r2 (137.7 MHz, 17.955 ms kernel / 18.053 ms TPOT, job 3454)** — same netlist as Iter67c, both auto-scaled because they fail their 6.667 ns constraint. See *Kernel frequency*. **Iter76 (2026-09-12): the 150 MHz build recipe and its two-job Slurm flow are committed and demonstrated twice from source (see *In flight*); `make run_hw` now defaults to HLS 150 / link 150 with `hw_f150.cfg`** |
| In flight | **Iter73** (source-side campaign for a timing-closed 150 MHz, user-approved 2026-09-06): 73a + 73b2 passed csynth and one-layer RTL cosim; the first 150 MHz production link (3491, 2026-09-07) **did not route** (11,197 overlaps). Census 3516 on its placement: both fixes worked at placement (state-write stall cone gone, cluster spill halved), SLR0 was *less* full than the legal V0 draw, and the new anomaly is tearing — cluster 2 split 35K/11K, `mm0` across three SLRs. Attribution complete (jobs 3522/3537, 11,192 conflicting nodes = Vivado's 11,197 overlaps): **98.1% of conflicts are on non-crossing nets, 0% on SLL/Laguna, 95.5% in SLR0**, half owned by the shell's HBM switch and 17.3% by free-placed cluster 4 sharing its switch boxes at X120-137/Y40-70. **Iter74 (jobs 3557/3558/3559) then settled it: all three re-placements of that netlist route legally, and the control closes 150 MHz — kernel WNS +0.005 ns, 0 failing of 1.58M, DMA +0.002, hold +0.007.** 3491 was a bad placement draw; the recommended cluster-4 re-pin proved unnecessary (−0.029) and a 4+9 re-pin harmful (−0.387). Production link draw 2 (build 3661) **FAILED identically to 3491** — 11,197 node overlaps, 15,197 unroutable signals, same congestion levels — proving the production `v++` link is **deterministic** for identical inputs, so a repeat draw is not a lever; W0 routed only because a re-placement from a checkpoint is a different placement problem. A shortcut that spliced W0's partial bitstream into an older image's metadata **failed on card** (jobs 3665/3666: deterministically wrong logits, byte-identical at 150 and 137 MHz, so not a clock-margin effect). Job 3667 proved the splice **mechanism** sound (re-spliced donor passes both gates, 17.960 ms), and the user's card test then found the real cause: **the W0 image never updates the recurrent state in HBM**. So checkpoint reuse is a *validated* 34-minute substitute for a 9–18 h link, and the defect is a hardware-only failure of the **Iter73a** state write-back path — invisible to csim and one-layer cosim, and the second such failure from refactoring that path (cf. Iter68G). 73b2 does not touch it. **Iter75 (build 3675) then isolated the compiler stage in 48 min via a new pre-link `state_address_gate`: the HLS RTL computes the address correctly (48/48 pass) and the post-synthesis netlist is still 128 MiB low (48/48 fail) — Vivado's kernel synthesis introduces it, so source-level restatement (`ap_uint<64>`) does not fix it.** **Iter75b then FIXED it and PASSED ON CARD (job 3703): a registered-writer formulation survives synthesis where `ap_uint<64>` did not.** Exact 64-token trajectory, NRMSE 0.00466 / cosine 0.99999 identical to Iter67c, **17.233 ms kernel / 17.343 ms TPOT at 142.5 MHz = 2.4557M cycles** — fastest measured image (−28.5% vs committed Iter67c, −4.0% vs Iter72 r2). It also **routed legally**, breaking the deterministic route failure of 3491/3661. Not promoted: the clock auto-scaled 150 → 142.5 (missed by −0.349 ns, 1,733 failing endpoints), and 512-token drift / WikiText-2 are unrun. Image preserved at `diagnostics/iter75b_recover/gdn_forward_f142.xclbin` (`d48c6b15…`); the 150 MHz residual is now pure timing. **Correctness qualification is COMPLETE (job 3704): 512-token drift BOUNDED with the fork at step 447 — identical to every prior image — and WikiText-2 word PPL 16.774839771 vs GPU 16.776123769 = −0.0077% on verified-identical workload, reproducing the Iter66o 100 MHz card result to 9 significant figures.** Remaining bars: the auto-scaled clock (rule 4) and the paired latency+power protocol, which has never run on this image, so no J/token figure existed. **Both bars then fell (2026-09-10, Iter75e/f): a focused post-route pass on the −0.001 checkpoint closed 150 MHz outright — kernel setup 0.000 / hold +0.002 ns, DMA +0.003, HBM +0.052, 0 failing of 1.34M, no false paths, relaxed uncertainty or multicycle exceptions — and the packaged image PASSED ON CARD (job 3957): exact 64-token trajectory, identical logits gate, 16.131 ms kernel / 16.256 ms TPOT = 2.4197M cycles; paired power 48.0 W, 0.779 J/token gross, 0.357 J idle-subtracted (jobs 3958/3959), 1.94×/3.88× vs the paired A100 FP32 run.** **Iter76 (2026-09-10→12) then made it reproducible from source: the consolidated `make run_hw` recipe (`hw_f150.cfg` + pre-place control replication + in-link AggressiveExplore → Explore → focused kernel-group pass in `finish_f150_timing.tcl` + fail-closed exact-clock gate + `reconcile_exact_clock.py` for the DATA_CLK metadata that vpl scales from its pre-hook timing report) reproduced build 3751's implementation at every Vivado phase checksum in builds 3987 and 4022, closed 150 MHz in-link, and both images passed on card (jobs 4021/4023, 16.131 ms kernel / 16.255 ms TPOT, NRMSE 0.00466, zero argmax mismatches). One identical-input link (3968) diverged inside the placer's physical synthesis while other users' jobs shared its node and missed by 0.045 ns; `BUILD_EXCLUSIVE=user` is part of the recommended launch (one divergent sample — a grounded recommendation, not a proven mechanism). Flow committed. The 150 MHz design is fully qualified (drift, WikiText-2, power, reproducibility), but **promoting it as the production image in `architecture.md` and the status rows above is the user's call**; `architecture.md` still describes Iter67c |
...
| Document | Status |
|---|---|
| `c_impl/doc/architecture.md` | **Authoritative** description of the production kernel (currently **Iter67c**) — data flow, HBM map, per-layer schedule, physical design, measured result. Start here. |
...
| `c_impl/doc/cycle_optimization_roadmap.md` | **Proposed** future stages. Targets, not implemented hardware. Its cycle reference is deliberately still **Iter67c's 2.4099M**, not the faster Iter75b image, because Iter75b's cycles are 1.9% *worse* — every gain above 100 MHz is clock, not schedule, so frequency and cycle levers are not additive. |
...
- **The measured ladder** (each step on-card): 121.4 ms (8-port reference) → 98.66 (Iter32, activation resident) → 75.06 (Iter35, DMA fanout repair) → 59.58 (Iter36, head-local recurrence) → 51.45 (Iter37, four state ports) → 47.08 (Iter38, merged layouts/concurrent state) → 43.09 (Iter39C, head-streamed convolution) → 42.02 (Iter57, timing-friendly recurrent islands) → 42.17 (Iter61, on-chip logit export) → 26.65 (Iter66e, all-BF16) → **24.21** (Iter67c, on-chip argmax + II=1 recurrent read). Total **5.01×** over the eight-port reference. Off the ladder because unpromoted (auto-scaled, same netlist as Iter67c): 20.10 (Iter69, 121 MHz) and **17.96 (Iter72 r2, 137.7 MHz)**. Steps through Iter61 are bit-exact; Iter66e onward are exact-trajectory plus the CUDA vector gate — see the gate note under *Decode correctness*.
  **Kernel-only, like for like: 25.625 → 24.099 ms, −6.0%.** The ladder's earlier entries are wall times under older, looser timer boundaries, so treat the last two steps as the only strictly comparable pair.
...
- **At the current 100 MHz, the design is not HBM-bound — always re-evaluate this at a new clock.** At 2.799 GB of weights per token, 32 ports × 64 B × 100 MHz gives **1,366,528 beat-cycles per port against a measured 2,562,500** — the ports are busy **53.3%** of the token. The microbench sustains 98.353% of clock-rate ceiling on this exact port structure, so the other half is scheduling, not memory. This is also why BF16 delivered 1.582× and not 2× at 100 MHz. The standing correction is clock-scoped: halving bytes does not halve the current token, but at 200 MHz each port already requests 88.9% of its 14.4 GB/s pseudo-channel peak, and at 250 MHz it requests 111.1%. Compression regains first-order leverage once the design approaches that wall.
- **Open levers, in order of measured promise.** (1) **Sub-byte weight compression** (INT4/INT3, non-power-of-two groups with dequantisation in the datapath) is still the only lever with the right magnitude, and it is now the *only* one above 10%: it would take weight beats from 1,366,528 to ~341,632 per port (the 37% / 25.6 → ~16.1 ms estimate was derived from the old 1,268,224 figure and needs re-deriving), and it is something tensor cores cannot natively execute. (2) **The recurrent block is the new second target** — 43,427 cycles/layer × 24 = **1.042M cycles, 40.7% of the token**, and it did *not* shrink when the weights did. Its state movement alone (`recur_island_load_state` 1,027 + `recur_island_update` 1,035, per head, × 192 head-steps) is **395,904 cycles = 15.4%**. (3) The cycle roadmap's remaining stages — head-chunked output projection, then chunk-streamed GU/SwiGLU/MLP-down — remain percent-level. (4) **Frequency, but only through the Iter73 source campaign** — 137.7 MHz already runs on card at 17.955 ms (−25.5%) and is unpromoted because it is auto-scaled; timing closure at 150 MHz is the open work. Levers (1) and (4) compound. Preserve the physical island, frp, and registered-boundary structure until a replacement completes on-card validation.
