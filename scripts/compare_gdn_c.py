...
def main() -> None:
    ...
    parser.add_argument("--decode-golden", type=Path)
    ...
    if args.decode_golden is not None:
        if args.output is None:
            raise ValueError("--decode-golden requires --output for the golden JSON path.")
        model, device = load_model(args.model_name, args.device, args.dtype)
        decode_golden(model, device, args.decode_golden, args.output)
        return

    if not args.fixture:
        raise ValueError("Provide --fixture (scoring mode) or --decode-golden (decode mode).")
