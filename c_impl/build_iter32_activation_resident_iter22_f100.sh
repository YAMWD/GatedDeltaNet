...
expected_source_sha256=5a4f079a508c71d476a101776cd6285970230cdb4c5be6a1e0f9a486ac5f21e9
...
config="${c_impl_dir}/hw_iter32_activation_resident_iter22_postphys_f100.cfg"
expected_config_sha256=0531b6a1856de85425d8131589eb829988eaab37026cdb3c8a80189a40e3ed02
...
if [[ ! -f "${vitis_settings}" ]]; then
    echo "iter32: Vitis settings script not found: ${vitis_settings}" >&2
    exit 1
fi
...
if [[ ! -x "${vpp}" ]]; then
    echo "iter32: Vitis v++ not found or not executable: ${vpp}" >&2
    exit 1
fi
...
verify_sha256() {
    ...
    if [[ "${actual}" != "${expected}" ]]; then
        echo "iter32: ${label} hash mismatch: expected ${expected}, got ${actual}" >&2
        exit 1
    fi
}
...
if [[ -e "${build_dir}" ]]; then
    echo "iter32: refusing to overwrite existing build directory: ${build_dir}" >&2
    exit 1
fi
...
{
    printf "iteration=iter32_activationresident_iter22_postphys_f100\n"
    printf "change=activation-resident Pack16 datapath; in-place convolution; local residual accumulation; direct LM-head argmax\n"
    printf "source=%s\n" "${source_file}"
...
if [[ -n "${expected_xo_sha256}" &&
    ...
    echo "iter32: compiled XO hash mismatch: expected ${expected_xo_sha256}, got ${actual_xo_sha256}" >&2
    exit 1
fi
{
...
echo "iter32 build complete: ${build_dir}/gdn_forward.xclbin"
